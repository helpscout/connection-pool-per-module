...
import helpscout.shared.config.ModuleName.USER
import helpscout.shared.config.RoutingDataSourceTransactionInterceptors
import helpscout.user.UserDataSourceIntegrationTest.TestController
...
class UserDataSourceIntegrationTest {
    ...
    @Test
    fun `test list users endpoint`() {
        mvc.perform(get("/test/users").contentType(APPLICATION_JSON))
    ...
    class TestController {
        @GetMapping("/users")
        fun listUsers() {
            assertThat(DataSourceContext.current).isEqualTo(USER)
        }
    }
}